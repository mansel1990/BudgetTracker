...
const History = ({ userSettings }: { userSettings: user_settings }) => {
  ...
  const formatter = useMemo(() => {
    return GetFormatterForCurrency(userSettings.currency ?? "INR");
  ...
  const historyDataQuery = useQuery({
    ...
    queryFn: () =>
      fetch(
        `/api/history-data?timeframe=${timeframe}&year=${
          Number(period.year) || 0
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex w-full justify-between">
        <p className={cn("text-sm text-muted-forefround")}>{label}</p>
